...
if ( publisher_get_prop( 'show-listing-wrapper', TRUE ) ) {
	?>
	<div class="listing listing-blog listing-blog-1 clearfix <?php publisher_echo_prop( 'listing-class' ); ?> columns-<?php publisher_echo_prop( 'listing-columns', 1 ) ?>">
	<?php
}
...
if ( ! publisher_get_prop( 'block-customized', FALSE ) && publisher_have_posts() ) {
	...
	$block_settings = publisher_get_option( 'listing-blog-1' );
	...
	if ( publisher_get_prop( 'block-ad', FALSE ) ) {
		publisher_set_prop( 'block-ad-class', 'listing-item-blog listing-item-blog-1' );
	}
